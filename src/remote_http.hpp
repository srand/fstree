...
class remote_http : public remote {
 public:
  ...
  virtual bool has_object(const std::string& hash) override;
  ...
  virtual void has_objects(const std::vector<std::string>& hashes, std::vector<bool>& presence) override;

  // Send the object with the given hash and path to the remote.
  virtual void write_object(const std::string& hash, const std::filesystem::path& path) override;
 ...
 private:
    ...
    bool head(const std::string& hash);
